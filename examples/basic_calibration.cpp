...
int main(int argc, char** argv) {
  ...
  if (result.success) {
    ...
    std::cout << "RMSE: " << std::fixed << std::setprecision(6) << result.rmse
              << " m" << std::endl;

    std::cout << std::endl
              << "Estimated transformation (T_camera_lidar):" << std::endl;
    ...
    for (int i = 0; i < 4; ++i) {
      for (int j = 0; j < 4; ++j) {
        float err =
            std::abs(result.transformation(i, j) - T_ground_truth(i, j));
        if (err > max_error) max_error = err;
      }
    }
    std::cout << std::endl
              << "Max element-wise error: " << max_error << std::endl;
