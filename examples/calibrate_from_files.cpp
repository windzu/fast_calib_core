...
bool loadCameraIntrinsics(const std::string& filename, CameraIntrinsics& cam) {
  ...
  while (std::getline(file, line)) {
    ...
    try {
      if (key == "fx" || key == "cam_fx")
        cam.fx = std::stod(value);
      else if (key == "fy" || key == "cam_fy")
        cam.fy = std::stod(value);
      else if (key == "cx" || key == "cam_cx")
        cam.cx = std::stod(value);
      else if (key == "cy" || key == "cam_cy")
        cam.cy = std::stod(value);
      else if (key == "k1" || key == "cam_d0")
        cam.k1 = std::stod(value);
      else if (key == "k2" || key == "cam_d1")
        cam.k2 = std::stod(value);
      else if (key == "p1" || key == "cam_d2")
...
bool loadPointCloud(const std::string& pcd_file, LiDARType lidar_type,
  ...
  if (lidar_type == LiDARType::Solid) {
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ>(pcd_file, *xyz_cloud) == -1) {
      std::cerr << "Failed to load point cloud as XYZ: " << pcd_file
                << std::endl;
      return false;
    }
    ...
    std::cout << "Loaded XYZ point cloud (solid-state mode): " << cloud->size()
              << " points" << std::endl;
  } else {
    // Mechanical LiDAR: load XYZRing format directly
    if (pcl::io::loadPCDFile<PointXYZRing>(pcd_file, *cloud) == -1) {
      std::cerr << "Failed to load point cloud as XYZRing: " << pcd_file
                << std::endl;
      std::cerr << "Note: Mechanical LiDAR requires PCD with 'ring' field"
                << std::endl;
      return false;
    }
...
void printUsage(const char* progname) {
  std::cout << "Usage: " << progname
            << " <pcd_file> <image_file> <camera_yaml> [options]" << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "  --lidar-type <type>  LiDAR type: 'solid' (default) or 'mech'"
            << std::endl;
  std::cout
      << "                       solid: Solid-state LiDAR (e.g., Livox Mid360)"
      << std::endl;
  std::cout << "                       mech:  Mechanical LiDAR (e.g., "
               "Velodyne, Ouster)"
            << std::endl;
  std::cout
  ...
  std::cout << "  " << progname << " scan.pcd image.png camera.yaml"
            << std::endl;
  std::cout << "  " << progname
            << " scan.pcd image.png camera.yaml --lidar-type solid"
            << std::endl;
  std::cout
      << "  " << progname
      << " scan.pcd image.png camera.yaml --lidar-type mech --output ./results"
      << std::endl;
}
...
int main(int argc, char** argv) {
  ...
  std::cout << "==========================================" << std::endl;
  std::cout << "LiDAR type: "
            << (lidar_type == LiDARType::Solid ? "Solid-state" : "Mechanical")
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "  k1=" << camera.k1 << ", k2=" << camera.k2 << std::endl
            << std::endl;
  ...
  CalibrationResult calib_result =
      calculator.compute(lidar_result.centers, qr_result.centers);

  if (!calib_result.success) {
    std::cerr << "Calibration failed: " << calib_result.error_message
              << std::endl;
    return 1;
  }
  ...
  projectPointCloudToImage(cloud, calib_result.transformation,
  ...
  if (saver.saveCalibrationResults(params, calib_result, colored_cloud,
                                   qr_result.annotated_image)) {
