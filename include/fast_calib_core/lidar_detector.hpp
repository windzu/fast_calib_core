...
/**
 * @brief Validates if a set of 4 points forms a rectangle with expected dimensions
 */
...
public:
    ...
    LiDARDetectionResult detectMechanical(const PointCloudRingPtr& cloud) {
        ...
        log(LogLevel::Info, "Plane cloud size: " + 
            std::to_string(result.plane_cloud->size()));
        ...
        extractEdgePointsMech(result.filtered_cloud, plane_coeffs, 
                              result.edge_cloud, result.aligned_cloud,
                              R_align, average_z);
        log(LogLevel::Info, "Extracted " + std::to_string(result.edge_cloud->size()) + 
    ...
    /**
     * @brief Detect circle centers for solid-state LiDAR (without ring information)
    ...
    LiDARDetectionResult detectSolidState(const PointCloudRingPtr& cloud) {
        ...
        log(LogLevel::Info, "After voxel filter: " + 
            std::to_string(result.filtered_cloud->size()));
        ...
        Eigen::Matrix3d R_align;
        float average_z;
        alignPlaneToZ0(result.plane_cloud, plane_coeffs, 
                       result.aligned_cloud, R_align, average_z);
...
private:
    ...
    void filterCloud(const PointCloudRingPtr& input, PointCloudRingPtr& output) {
        ...
        pass_x.setFilterLimits(params_.lidar_filter.x_min, params_.lidar_filter.x_max);
        pass_x.filter(*output);

        pcl::PassThrough<PointXYZRing> pass_y;
        pass_y.setInputCloud(output);
        pass_y.setFilterFieldName("y");
        pass_y.setFilterLimits(params_.lidar_filter.y_min, params_.lidar_filter.y_max);
        pass_y.filter(*output);

        pcl::PassThrough<PointXYZRing> pass_z;
        pass_z.setInputCloud(output);
        pass_z.setFilterFieldName("z");
        pass_z.setFilterLimits(params_.lidar_filter.z_min, params_.lidar_filter.z_max);
        pass_z.filter(*output);
    }

    // Fit plane using RANSAC
    bool fitPlane(const PointCloudRingPtr& input, 
                  PointCloudRingPtr& plane_cloud,
    ...
    void extractEdgePointsMech(const PointCloudRingPtr& filtered_cloud,
        ...
        for (auto& kv : ring2indices) {
            ...
            for (size_t k = 1; k + 1 < idx_vec.size(); ++k) {
                ...
                double dist_plane = std::fabs(plane_coeffs[0]*p_cur.x + 
                                             plane_coeffs[1]*p_cur.y + 
                                             plane_coeffs[2]*p_cur.z + 
                                             plane_coeffs[3]) / norm_n;
        ...
        for (const auto& pt : *edge_cloud) {
            ...
            aligned_cloud->push_back(pcl::PointXYZ(aligned_point.x(), 
                                                    aligned_point.y(), 0.0));
    ...
    void fitCirclesToClusters(const PointCloudXYZPtr& edge_cloud,
        ...
        for (const auto& cluster : cluster_indices) {
            ...
            if (inliers->indices.size() > 0) {
                ...
                for (const auto& idx : inliers->indices) {
                    ...
                    double distance = sqrt(dx*dx + dy*dy) - params_.target.circle_radius;
    ...
    void generateCombinations(int N, int K, std::vector<std::vector<int>>& groups) {
        groups.clear();
        if (N < K || K <= 0) return;
    ...
    bool selectAndTransformCenters(const PointCloudXYZPtr& center_z0_cloud,
        ...
        for (size_t i = 0; i < groups.size(); ++i) {
            ...
            SquareValidator validator(candidates, 
                                      params_.target.delta_width_circles,
        ...
        if (valid_count > 1) {
            log(LogLevel::Error, "More than one set of candidates fit target's geometry.");
            return false;
        }

        if (best_idx == -1) {
            log(LogLevel::Warning, "Unable to find a candidate set that matches target's geometry");
            return false;
        }
        ...
        for (int j : groups[best_idx]) {
            ...
            Eigen::Vector3d aligned_point(center_z0.x, center_z0.y, center_z0.z + average_z);
            Eigen::Vector3d original_point = R_inv * aligned_point;
