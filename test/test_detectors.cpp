...
class SquareValidatorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    perfect_square->push_back(
        pcl::PointXYZ(0.0f, 0.0f, 0.0f));  // 0: Bottom-left
    perfect_square->push_back(
        pcl::PointXYZ(0.3f, 0.0f, 0.0f));  // 1: Bottom-right
...
TEST_F(SquareValidatorTest, WrongSideLength) {
  ...
  SquareValidator validator;
  EXPECT_FALSE(validator.validate(*perfect_square,
                                  0.5));  // Expected 0.5m, but square is 0.3m
}
