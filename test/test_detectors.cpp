...
// ============================================================================
// SquareValidator Tests
// ============================================================================

class SquareValidatorTest : public ::testing::Test {
protected:
    void SetUp() override {
        // Create a perfect square of points (0.3m x 0.3m)
        perfect_square.reset(new PointCloudXYZ);
        perfect_square->push_back(pcl::PointXYZ(0.0f, 0.0f, 0.0f));   // 0: Bottom-left
        perfect_square->push_back(pcl::PointXYZ(0.3f, 0.0f, 0.0f));   // 1: Bottom-right
        perfect_square->push_back(pcl::PointXYZ(0.3f, 0.3f, 0.0f));   // 2: Top-right
        perfect_square->push_back(pcl::PointXYZ(0.0f, 0.3f, 0.0f));   // 3: Top-left
    }

    PointCloudXYZPtr perfect_square;
};

TEST_F(SquareValidatorTest, PerfectSquare) {
    SquareValidator validator;
    EXPECT_TRUE(validator.validate(*perfect_square, 0.3));
}

TEST_F(SquareValidatorTest, SquareWithNoise) {
    // Add small noise
    PointCloudXYZPtr noisy_square(new PointCloudXYZ);
    for (const auto& pt : perfect_square->points) {
        pcl::PointXYZ noisy_pt = pt;
        noisy_pt.x += 0.001f;  // 1mm noise
        noisy_pt.y += 0.001f;
        noisy_square->push_back(noisy_pt);
    }
    
    SquareValidator validator(0.1);  // 10% tolerance
    EXPECT_TRUE(validator.validate(*noisy_square, 0.3));
}

TEST_F(SquareValidatorTest, Rectangle) {
    // Create a rectangle (not a square)
    PointCloudXYZPtr rectangle(new PointCloudXYZ);
    rectangle->push_back(pcl::PointXYZ(0.0f, 0.0f, 0.0f));
    rectangle->push_back(pcl::PointXYZ(0.4f, 0.0f, 0.0f));  // Different width
    rectangle->push_back(pcl::PointXYZ(0.4f, 0.3f, 0.0f));
    rectangle->push_back(pcl::PointXYZ(0.0f, 0.3f, 0.0f));
    
    SquareValidator validator;
    EXPECT_FALSE(validator.validate(*rectangle, 0.3));
}

TEST_F(SquareValidatorTest, TooFewPoints) {
    PointCloudXYZPtr three_points(new PointCloudXYZ);
    three_points->push_back(pcl::PointXYZ(0.0f, 0.0f, 0.0f));
    three_points->push_back(pcl::PointXYZ(0.3f, 0.0f, 0.0f));
    three_points->push_back(pcl::PointXYZ(0.3f, 0.3f, 0.0f));
    
    SquareValidator validator;
    EXPECT_FALSE(validator.validate(*three_points, 0.3));
}

TEST_F(SquareValidatorTest, WrongSideLength) {
    // Perfect square but wrong expected size
    SquareValidator validator;
    EXPECT_FALSE(validator.validate(*perfect_square, 0.5));  // Expected 0.5m, but square is 0.3m
}
